...
def take_name():
    while True:
        ...
        nam = takecommand()
        if nam:
            say(f"Hello, {nam}! How may I help you today? If you are in trouble say Safeguard")
            print("If you are in trouble say Safeguard ")
            return nam

def send_message(contacts, message):
    for contact in contacts:
        if is_whatsapp_password_set(contact):
            send_whatsapp_message(contact, message)
        else:
            send_sms(contact, message)

def safeguard_mode(emergency_contacts):
    while True:
        # Listen for user input
        command = takecommand().lower()

        # Check if the user said "safeguard"
        if "safeguard" in command:
            say("Don't worry! We have activated trigger mode. Sending location to all emergency contacts...")
            location = get_current_location()  # Function to get the current location
            message = f"Emergency! I'm in trouble. My current location is {location}. Please help!"
            send_message(emergency_contacts, message)
            break  # Exit safeguard mode loop

# Example usage
def main():
    emergency_contacts = ["+1234567890", "+0987654321"]  # Replace with actual emergency contacts
    user_name = take_name()
    safeguard_mode(emergency_contacts)
